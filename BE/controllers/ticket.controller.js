...
const getTickets = async (req, res) => {
  try {
    ...
    if (isAssignedTo) {
      // Every ticket assigned to (or, failing that, created by) this employee,
      // regardless of status — a freshly assigned ticket starts as "pending"
      // and was previously invisible to its own assignee until someone else
      // moved it along, which also made it impossible for them to open/edit it.
      filter.$or = [
        { assignedEmployee: user.id },
        { userId: user.id, assignedEmployee: null },
      ];
    }
...
const updateTicket = async (req, res) => {
  try {
    ...
    if (isEmployee) {
      ...
      if (updates.client) {
        const clientDoc = await Client.findById(updates.client);
        if (!clientDoc || !clientDoc.isActive) {
          return res.status(400).json({
            success: false,
            message: "Invalid or inactive client.",
